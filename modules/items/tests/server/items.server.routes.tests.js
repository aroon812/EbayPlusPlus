...
describe('Item CRUD tests', function () {
  ...
  it('should be able to get a single item if not signed in', function (done) {
    ...
    // Save the item
    itemObj.save(function () {
      agent.get('/api/items/' + itemObj._id)
        .end(function (req, res) {
          // Set assertion
  ...
  it('should be able to get a single item if not signed in and verify the custom "isCurrentUserOwner" field is set to "false"', function (done) {
    // Create new item model instance
    var itemObj = new Item(item);
